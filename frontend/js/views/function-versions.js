...
export const FunctionVersions = {
    ...
    loadData: async (id) => {
        ...
        try {
            const [func, versions] = await Promise.all([
                ...
                API.versions.list(id, FunctionVersions.versionsLimit, FunctionVersions.versionsOffset),
            ]);
    ...
    compareVersions: () => {
        ...
        m.route.set(`/functions/${FunctionVersions.func.id}/diff/${v1}/${v2}`);
    },
                                ...
                                m(Table, [
                                    ...
                                    m(TableBody,
                                        FunctionVersions.versions.map(ver =>
                                            m(TableRow, {
                                                key: ver.version,
                                                class: FunctionVersions.selectedVersions.includes(ver.version) ? "table__row--selected" : ""
                                            }, [
                                                m(TableCell, [
                                                    m("input[type=checkbox]", {
                                                        checked: FunctionVersions.selectedVersions.includes(ver.version),
                                                        onchange: () => FunctionVersions.toggleVersionSelection(ver.version)
                                                    })
                                                ]),
                                                m(TableCell, { mono: true }, `v${ver.version}`),
                                                ...
                                                m(TableCell, [
                                                    ver.version !== func.active_version.version && m(Button, {
                                                        variant: ButtonVariant.SECONDARY,
                                                        size: ButtonSize.SM,
                                                        onclick: (e) => {
                                                            e.stopPropagation();
                                                            FunctionVersions.activateVersion(ver.version);
                                                        }
                                                    }, "Activate")
                                                ])
                                            ])
